...
def seleccion_formula(cursor):
    ...
    lista=[]
    cursor.execute("SELECT [id_Receta],[Codigo],[Descripcion] FROM  [dbo].[Receta]")
    row = cursor.fetchone()
    while row:
...
def seleccion_tolvas(cursor):
    ...
    lista=[]
    cursor.execute("SELECT [id_Tolva],[Tolva],[Codigo],[Descripcion] FROM  [dbo].[Tolva] WHERE Tolva LIKE 'PT%'")
    row = cursor.fetchone()
    while row:
...
def validar_tolva_pt(cursor,receta,tolva):
    ...
    cursor.execute("SELECT [Codigo] FROM  [dbo].[Tolva] WHERE [Tolva] = '{}'".format(tolva))
    row = cursor.fetchone()
    codigo = row[0]
    cursor.commit()
...
def actulizar_tolva_pt(cursor,receta,tolva):
    try:
        cursor.execute("SELECT [Codigo] ,[Descripcion] FROM  [dbo].[Receta] WHERE [Codigo] = {}".format(int(receta)))
        row = cursor.fetchone()
        value = { "codigo": row[0], "descripcion" : row[1] }
        cursor.commit()
...
def cambiar_registro_grasa(cursor,grasa,host,connector,client):
    try:
        cursor.execute("SELECT [RecetaActual].[Codigo], [RecetaActual].[Cantidad],[Tolva] FROM  [dbo].[RecetaActual] INNER JOIN Tolva ON Tolva.Codigo = RecetaActual.Codigo WHERE Tolva.Tolva like 'TA%'")
        row = cursor.fetchone()
        cursor.commit()
...
def consultar_registro_grasa(cursor):
    ...
    cursor.execute("SELECT [RecetaActual].[Descripcion],[RecetaActual].[Cantidad] FROM  [dbo].[RecetaActual] INNER JOIN Tolva ON Tolva.Codigo = RecetaActual.Codigo WHERE Tolva.Tolva like 'TA%'")
    row = cursor.fetchone()
...
def cambiar_string_plc(cursor, host,connector,client,diccionario):
    ...
    cursor.execute("SELECT [Receta].[Descripcion] FROM  [dbo].[Receta] WHERE [Receta].[Codigo] = {}".format(diccionario['nombre']))
    row = cursor.fetchone()
    nombre_real = row[0]
    cursor.commit()
...
def actulizar_ingManual(cursor,valores,host,connector,client):
    try:
        cursor.execute("SELECT [Codigo] ,[Descripcion] FROM  [dbo].[RecetaActual] WHERE [Codigo] = {}".format(int(valores['ingrediente'])))
        row = cursor.fetchone()
        value = { "codigo": row[0], "descripcion" : row[1] }
        cursor.commit()
